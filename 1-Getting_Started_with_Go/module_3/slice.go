...
During each pass through the loop, the program prompts the user to enter an integer to be added
to the slice. The program adds the integer to the slice, sorts the slice,
...
func main(){
	...
	for myScanner.Scan() {
		...
		if txt == "X" {
			...
			number, _ := strconv.ParseInt(txt, 10, 64)
			mySlice = append(mySlice, int(number))
			sort.Ints(mySlice)
		}
		fmt.Println(mySlice)
	}
	//io.WriteString(os.Stdout, strconv.FormatInt(result, 10))
}
